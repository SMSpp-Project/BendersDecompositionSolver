 ...
 *
 * Sherali and Lunday obtain a nondominated cut from one problem only, the
 * separation at the incumbent moved by a small step towards the core point,
 * \f[
 *   \bar{x} = \hat{x} + \mu ( x^\circ - \hat{x} ) \;,
 * \f]
 * whose optimal face is a single vertex for a small enough \f$ \mu \f$
 * (dbl_BDSlv_ParetoMu) and is one of the optimal ones at \f$ \hat{x} \f$;
 * on the instances measured \f$ \mu = 0.1 \f$ halves the rounds of the
 * static cut where \f$ 10^{-4} \f$ changes nothing, hence the default;
 * it is written for the unified cut only [see unified_cut_type], whose
 * separation problem is where a degenerate optimal face was measured, and
 * the cut is added when it is violated at the incumbent. */
 ...
 enum cut_strengthening_type {
  ...
  eSheraliLunday = 2   ///< the unified cut separated at a perturbed incumbent
  };
...
/*--------------------------------------------------------------------------*/
 /// whether one cut is separated for both feasibility and optimality
 /** The classical decomposition asks the subproblem two separate questions,
  * is it feasible at the incumbent (and, if not, which ray says so) and, if
  * it is, what does its value function look like there: the answers are two
  * families of cuts, generated by two different mechanisms and each blind to
  * what the other sees. Cornuejols and Lemarechal, and after them Fischetti,
  * Salvagnin and Zanette, Conforti and Wolsey, Brandenberg and Stursberg,
  * observe that the two conditions defining the epigraph of the value
  * function can be separated *together*, by one cut-generating problem over
  * the polyhedron
  * \f[
  *   \{ ( \pi , \pi_0 ) \geq 0 : \pi^{\top} R + \pi_0 c^{\top} \geq 0 \}
  * \f]
  * where R is the matrix of the subproblem and c the cost of its Objective:
  * the cut it yields, \f$ \pi^{\top} ( F x - u ) + \pi_0 ( c^{\top} y -
  * \eta ) \leq 0 \f$, is a feasibility cut when \f$ \pi_0 = 0 \f$ and an
  * optimality cut when \f$ \pi_0 > 0 \f$, and which of the two it is comes
  * out of the separation rather than being decided in advance. Such a cut is
  * called *unified*.
  *
  * The unified cut is separated here by the very problem the phase one
  * already is, with the epigraph inequality \f$ c^{\top} y \leq \eta \f$
  * added to the coupling Constraint that carry a slack: the multipliers of
  * the phase one are bounded by the costs of the slacks, so those costs are
  * the normalization of the cut-generating problem, and the cost of the
  * slack of the epigraph inequality [see dbl_BDSlv_EpiWeight] is the weight
  * \f$ \tilde{\omega}_0 \f$ of the literature. The value of that problem is
  * zero exactly at the points of the epigraph, its linearization in
  * \f$ ( x , \eta ) \f$, asked to be nonpositive, is the cut, and an
  * infeasible subproblem needs no certificate: it is one more point where
  * the value is positive. This requires the MILP regime, \f$ \eta \f$ being
  * a Variable of the master, and un-aggregated cuts, each cut carrying the
  * \f$ \eta \f$ of its own subproblem.
  *
  * Since the multipliers are bounded by the costs of the slacks, what decides
  * how the cut divides itself between the two roles is the *ratio* between
  * the cost of the slack of the epigraph inequality and those of the coupling
  * Constraint, and not either of them: the ordinary optimality cut is the
  * limiting case where \f$ \pi_0 = 1 \f$ and \f$ \pi \f$ is free, i.e., where
  * that ratio goes to zero, while a ratio of one on a model whose Objective
  * is in money and whose coupling Constraint are in physical units caps the
  * multipliers of the latter far below the duals of the subproblem, and
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 enum dbl_par_type_BDSlv {
  ...
  dbl_BDSlv_ParetoMu ,
  ///< step towards the core point of the cut of Sherali and Lunday, in ( 0 , 1 )
